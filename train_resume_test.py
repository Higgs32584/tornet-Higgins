...
def main(config):
    ...
    ds_train = get_dataloader(dataloader, DATA_ROOT, config['train_years'], "train", batch_size, {}, **dataloader_kwargs)
    ds_val = get_dataloader(dataloader, DATA_ROOT, config['val_years'], "train", batch_size, {}, **dataloader_kwargs)
    ...
    nn = build_model_exp(shape=in_shapes, c_shape=c_shapes, start_filters=config['start_filters'], 
                         l2_reg=config['l2_reg'], input_variables=input_variables, model=config['model'])
    print(nn.summary())
    
    # Loss & Optimizer
    loss = keras.losses.BinaryCrossentropy(label_smoothing=config['label_smooth'])
    lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(learning_rate, decay_steps, decay_rate)
    ...
    # TensorBoard Logging
    tensorboard_cb = keras.callbacks.TensorBoard(log_dir=os.path.join(expdir, 'logs'), write_graph=False)
    ...
    for epoch in range(epochs):
        if check_spot_instance():
            logging.warning("Spot instance interruption detected! Saving checkpoint and exiting...")
            nn.save_weights(checkpoint_path)
            break
        
        history = nn.fit(ds_train, validation_data=ds_val, callbacks=callbacks, epochs=1, verbose=1)
