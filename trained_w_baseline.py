...
def get_or_create_baseline_preds(
    baseline_model, dataset, year, split, output_dir="baseline_preds"
):
...
def add_baseline_probs_to_dataset(dataset, baseline_probs):
    def gen():
        ...
        for x, y, _ in dataset:
            ...
            baseline_tensor = tf.reshape(
                tf.convert_to_tensor(baseline_slice, dtype=tf.float32), (batch_size, 1)
            )
    ...
    input_spec = {
        k: tf.TensorSpec(shape=v.shape, dtype=v.dtype)
        for k, v in input_sample[0].items()
    }
    ...
    output_spec = tf.TensorSpec(
        shape=input_sample[1].shape, dtype=input_sample[1].dtype
    )

    return tf.data.Dataset.from_generator(
        gen, output_signature=(input_spec, output_spec)
    )
...
def main(config):
    ...
    for year in train_years:
        ...
        train_probs.append(
            get_or_create_baseline_preds(baseline_model, ds_year, year, "train")
        )
