...
class FastNormalize(tf.keras.layers.Layer):
    def __init__(self, mean, std, **kwargs):
        super().__init__(**kwargs)
        self._mean_list = (
            [float(m) for m in mean.numpy()] if hasattr(mean, "numpy") else list(mean)
        )
        self._std_list = (
            [float(s) for s in std.numpy()] if hasattr(std, "numpy") else list(std)
        )
...
if __name__ == "__main__":
    ...
    p1 = mp.Process(
        target=load_model,
        args=("model1.json", "model1.weights.h5", "model1", return_dict),
    )
    p2 = mp.Process(
        target=load_model,
        args=("model2.json", "model2.weights.h5", "model2", return_dict),
    )
    ...
    ensemble_output = keras.layers.Average(name="soft_vote")(
