...
def build_model(
    ...
    x, c = wide_resnet_block(
        x, coords, filters=start_filters, l2_reg=l2_reg, drop_rate=dropout_rate
    )
    x = se_block(x)
    ...
    attn = Dropout(rate=0.2, name="attention_dropout")(attn)
    x = Multiply()([x, attn])
...
class MonitorPReLU(tf.keras.callbacks.Callback):
    def __init__(self, threshold=1e-4, verbose=True):
        super().__init__()
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        for layer in self.model.layers:
            if isinstance(layer, tf.keras.layers.PReLU):
                ...
                if max_abs_alpha < self.threshold:
                    relu_like_layers.append((layer.name, max_abs_alpha))

        if self.verbose:
            print(f"\n📊 Epoch {epoch + 1}:")
            if relu_like_layers:
                for name, val in relu_like_layers:
                    print(f"⚠️  Layer '{name}' acting like ReLU (max |α| = {val:.2e})")
            else:
                print(
                    "✅ All PReLU layers learning non-zero α (not behaving like ReLU)"
                )
...
DEFAULT_CONFIG = {
    ...
    "model": "wide_resnet",
    "start_filters": 96,
    "learning_rate": 1.27e-04,
    "l2_reg": 4.6e-5,
    "dropout_rate": 5.3e-2,
    "label_smoothing": 3.7e-2,
    "decay_steps": 1386,
    "decay_rate": 0.958,
    "wN": 1.0,
    "w0": 1.0,
...
def main(config):
    ...
    steps_per_epoch = config.get("first_decay_steps")
    ...
    initial_weights_path = os.path.join(".", "initial.weights.h5")

    # Save initial weights if they don't exist yet
    if not os.path.exists(initial_weights_path):
        nn.save_weights(initial_weights_path)

    # Load initial weights to reset model before training
    nn.load_weights(initial_weights_path)

    nn.compile(loss=loss, metrics=metrics, optimizer=opt, jit_compile=True)
    callbacks = [
        keras.callbacks.TerminateOnNaN(),
        MonitorPReLU(threshold=1e-4),
