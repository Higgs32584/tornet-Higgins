...
def cbam_block(x, ratio=16):
    ...
    ca = Add()([shared_dense(avg_pool), shared_dense(max_pool)])
    ca = keras.layers.Activation("sigmoid")(ca)
    ca = Reshape((1, 1, filters))(ca)
    x = Multiply()([x, ca])

    # Spatial Attention
    avg_pool = keras.layers.Lambda(lambda x: tf.reduce_mean(x, axis=-1, keepdims=True))(
        x
    )
    max_pool = keras.layers.Lambda(lambda x: tf.reduce_max(x, axis=-1, keepdims=True))(
        x
    )
    sa = Concatenate(axis=-1)([avg_pool, max_pool])
    sa = Conv2D(1, kernel_size=7, padding="same", activation="sigmoid")(sa)
    x = Multiply()([x, sa])
    return x


def hr_module(x, c, filters=64):
    ...
    x_pool = MaxPool2D(pool_size=2)(x)
    c_pool = MaxPool2D(pool_size=2)(c)
    low, _ = CoordConv2D(
        filters * 2,
        kernel_size=3,
        padding="same",
    ...
    low = ReLU()(low)
    low = UpSampling2D(size=2, interpolation="bilinear")(low)
    low = Conv2D(filters, kernel_size=1, padding="same", use_bias=False)(low)
    low = BatchNormalization()(low)
    low = ReLU()(low)

    # Fuse
    fused = Add()([high, low])
    fused = cbam_block(fused)
    return fused, c
...
def build_model(
    ...
    x, c = hr_module(x, c, filters=start_filters)
    skip1 = x
    x = MaxPool2D(pool_size=2)(x)
    c = MaxPool2D(pool_size=2)(c)
    x, c = hr_module(x, c, filters=start_filters)
    skip2 = UpSampling2D(size=2, interpolation="bilinear")(x)
    ...
    x = BatchNormalization()(x)
    x = ReLU()(x)
    output = keras.layers.Dense(1, activation="sigmoid", dtype="float32")(x)
...
def main(config):
    ...
    print(nn.summary())
    from_logits = False
    steps_per_epoch = 1020  # adjust this to your actual steps
    total_epochs = 100
