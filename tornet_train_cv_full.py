...
def wide_resnet_block(
    x, c, filters=64, widen_factor=2, l2_reg=1e-6, drop_rate=0.0, downsample=False
):
    """Wide ResNet Basic Block with CoordConv2D (pre-activation style)"""
    shortcut_x, shortcut_c = x, c
    out_filters = filters * widen_factor
    stride = 2 if downsample else 1

    # First BN-ReLU-CoordConv
    x = BatchNormalization()(x)
    x = ReLU()(x)
    x, c = CoordConv2D(
        filters=out_filters,
    ...
    if shortcut_x.shape[-1] != out_filters or downsample:
