...
class TverskyLoss(tf.keras.losses.Loss):
    def __init__(
        self, alpha=0.5, beta=0.5, smooth=1e-6, from_logits=False, name="tversky_loss"
    ):
...
def main(config):
    ...
    opt = AdamW(
        learning_rate=CosineDecayRestarts(
            initial_learning_rate=lr, first_decay_steps=100, t_mul=2.0, m_mul=0.9
        ),
        weight_decay=1e-4,
        beta_1=0.9,
        beta_2=0.999,
    ...
    nn = keras.models.load_model(
        "/home/ubuntu/tornet-Higgins/40 epoch march 22nd overnight/tornadoDetector_030.keras"
    )
