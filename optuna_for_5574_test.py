...
def main(config, trial=None):
    ...
    dataloader_kwargs.update(
        {
            "select_keys": config["input_variables"]
    ...
    metrics = [
        ...
        tfm.TruePositives(from_logits=False),
        tfm.FalsePositives(from_logits=False),
        tfm.TrueNegatives(from_logits=False),
        tfm.FalseNegatives(from_logits=False),
        tfm.Precision(from_logits=False),
        tfm.Recall(from_logits=False),
        tfm.F1Score(from_logits=False),
    ]
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "tune":
        study = optuna.create_study(
            direction="maximize", sampler=optuna.samplers.TPESampler(multivariate=True)
        )
