...
def build_model(model='wide_resnet',shape:Tuple[int]=(120,240,2),
                ...
                include_range_folded:bool=True,dropout_rate=0.1,dense_units:int=64):
    ...
    x_dense = Dense(dense_units, activation='relu')(x_concat)
...
tf.config.optimizer.set_jit(False)  # Enable XLA (Accelerated Linear Algebra)
...
def objective(trial):
    ...
    # config["first_decay_steps"] = trial.suggest_int("first_decay_steps", 1000, 2500)*1.0  # Controls decay restart interval
    config["start_filters"] = trial.suggest_int("start_filters", 32, 96,step=16)
    config["dense_units"] = 2 ** trial.suggest_int("dense_units", 5, 8)
    ...
    model = build_model(
        ...
        l2_reg=config["l2_reg"], start_filters=config["start_filters"],dropout_rate=config["dropout_rate"],dense_units= config["dense_units"]
    )
    loss = combo_loss()
    ...
    # Log trial results
    log_trial(trial, best_aucpr,config)
...
def log_trial(trial, trial_results, config):
    """Logs individual trial results to a JSON file."""
    trial_data = {
        "trial_id": trial.number,
        "params": trial.params,  # Only Optuna-suggested params
        "config": config,        # Full config used for training
        "value": trial_results
    }
