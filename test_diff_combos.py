...
LOG_DIR = os.path.join(
    "loss_opt_scheduler_runs", datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
)
...
LOSSES = {
    ...
    "asymmetric_focal": lambda gamma_pos=2.0, gamma_neg=1.0, alpha=0.25: lambda y_true, y_pred: -tf.reduce_mean(
        y_true
        * alpha
        * tf.pow(1 - y_pred, gamma_pos)
        * tf.math.log(tf.clip_by_value(y_pred, 1e-7, 1.0))
        + (1 - y_true)
        * (1 - alpha)
        * tf.pow(y_pred, gamma_neg)
        * tf.math.log(tf.clip_by_value(1 - y_pred, 1e-7, 1.0))
    ),
    "log_cosh_dice": lambda smooth=1e-6: lambda y_true, y_pred: tf.math.log(
        tf.cosh(
            1
            - (
...
SCHEDULERS = {
    "cosine_decay": lambda lr: CosineDecayRestarts(
        initial_learning_rate=lr, first_decay_steps=2000
    ),
    "exp_decay": lambda lr: ExponentialDecay(
        initial_learning_rate=lr, decay_steps=1000, decay_rate=0.96
    ),
    "poly_decay": lambda lr: PolynomialDecay(
        initial_learning_rate=lr, decay_steps=2000, end_learning_rate=1e-5
    ),
}
...
def run_experiment(loss_name, optimizer_name, scheduler_name):
    ...
    callbacks = [
        keras.callbacks.EarlyStopping(
            monitor="val_AUCPR", patience=3, restore_best_weights=True
        )
    ]

    print(
        f"🔧 Training: Loss={loss_name}, Optimizer={optimizer_name}, Scheduler={scheduler_name}"
    )
    history = model.fit(
        ds_train,
    ...
    with open(
        os.path.join(LOG_DIR, f"{loss_name}_{optimizer_name}_{scheduler_name}.json"),
        "w",
    ) as f:
        ...
        for optimizer_name in OPTIMIZERS:
            for scheduler_name in SCHEDULERS:
                try:
                    ...
                    print(
                        f"❌ Skipped: OOM - {loss_name}, {optimizer_name}, {scheduler_name}"
                    )
