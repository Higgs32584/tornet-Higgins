...
def resnet_block(x, c, filters=64, ksize=3, l2_reg=1e-6, drop_rate=0.0):
    ...
    if x.shape[-1] != shortcut_x.shape[-1]:  # Check if channels mismatch
        shortcut_x = Conv2D(x.shape[-1], (1, 1), padding='same')(shortcut_x)  # Match channels
...
DEFAULT_CONFIG={"epochs":100, 
                ...
                "val_years": [2021, 2022], "batch_size": 128, "model": "resnet", 
                "start_filters": 48, 
                "learning_rate": 1e-3, 
                "decay_steps": 1386, 
                "decay_rate": 0.958, 
                "l2_reg": 0.001, "wN": 1.0, "w0": 5.0, "w1": 5.0, "w2": 5.0, "wW": 5.0, "label_smooth": 0.1, "loss": "cce", "head": "maxpool", "exp_name": "tornado_baseline", "exp_dir": ".", "dataloader": "tensorflow-tfds", "dataloader_kwargs": {"select_keys": ["DBZ", "VEL", "KDP", "RHOHV", "ZDR", "WIDTH", "range_folded_mask", "coordinates"]}}
