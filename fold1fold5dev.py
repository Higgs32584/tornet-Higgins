...
class FillNaNs(keras.layers.Layer):
    def __init__(self, fill_val, **kwargs):
        ...
        self.initial_fill_val = fill_val  # Store the raw Python value

    @tf.function(jit_compile=True)
    def call(self, x):
        fill_val = tf.cast(self.initial_fill_val, x.dtype)
        return tf.where(tf.math.is_nan(x), fill_val, x)

    def get_config(self):
        return {**super().get_config(), "fill_val": self.initial_fill_val}


@keras.utils.register_keras_serializable()
class AttentionPooling2D(tf.keras.layers.Layer):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def build(self, input_shape):
        _, h, w, c = input_shape
        self.attention_conv = keras.layers.Conv2D(
            filters=1, kernel_size=1, activation="softmax", name="attention_weights"
        )

    def call(self, inputs):
        attn_logits = self.attention_conv(inputs)  # (B, H, W, 1)

        attn_logits_flat = tf.reshape(attn_logits, [tf.shape(inputs)[0], -1])
        attn_weights_flat = tf.nn.softmax(attn_logits_flat, axis=-1)
        attn_weights = tf.reshape(attn_weights_flat, tf.shape(attn_logits))
        weighted_inputs = inputs * attn_weights  # shape: (B, H, W, C)
        output = tf.reduce_sum(weighted_inputs, axis=[1, 2])  # shape: (B, C)
        return output
...
def wide_resnet_block(
    ...
    if drop_rate > 0:
        x = SpatialDropout2D(drop_rate, data_format="channels_last")(x)
...
class FastNormalize(keras.layers.Layer):
    ...
    def call(self, x):
        mean = tf.cast(self._mean, x.dtype)
        std = tf.cast(self._std, x.dtype)
        return (x - mean) / (std + tf.cast(1e-6, x.dtype))
...
def multi_dilated_attention_head(x, dilation_rates=[1, 2, 4], name_prefix="attn"):
    ...
    # context = AttentionPooling2D(name=f"{name_prefix}_context")(x)
    # context = GeMPooling2D()(x)
...
def multi_dilated_attention_block(
    ...
    # context = AttentionPooling2D()(x)

    # context = GeMPooling2D(name=f"{name_prefix}_gem_pool")(x)
