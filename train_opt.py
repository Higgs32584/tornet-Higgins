...
class CoordConv2D(keras.layers.Layer):
    def __init__(self, filters, kernel_size, kernel_regularizer, activation, padding='same', strides=(1,1), **kwargs):
        super().__init__(**kwargs)
        self.conv = Conv2D(filters, kernel_size, padding=padding, strides=strides,
                           kernel_regularizer=kernel_regularizer, activation=activation, use_bias=False)
...
def wide_resnet_block(x, c, filters, widen_factor=2, l2_reg=1e-6, drop_rate=0.0, nconvs=2):
    shortcut_x, shortcut_c = x, c
    for _ in range(nconvs):
        x, c = CoordConv2D(filters * widen_factor, 3, keras.regularizers.l2(l2_reg), activation=None)([x, c])
        x = BatchNormalization()(x)
    shortcut_x, _ = CoordConv2D(filters * widen_factor, 1, keras.regularizers.l2(l2_reg), activation=None)([shortcut_x, shortcut_c])
    x = Activation('relu')(x)
    x = Add()([x, shortcut_x])
...
def build_model(model='wide_resnet', shape=(120,240,2), c_shape=(120,240,2), input_variables=ALL_VARIABLES,
                start_filters=64, l2_reg=1e-3, background_flag=-3.0, include_range_folded=True, dropout_rate=0.1):
    ...
    if model == 'wide_resnet':
        for i in range(3):
            x, c = wide_resnet_block(x, c, start_filters * (2 ** i), l2_reg=l2_reg, drop_rate=dropout_rate)
        x = se_block(x)
...
def main(config):
    ds_train = get_dataloader(config['dataloader'], DATA_ROOT, config['train_years'], "train",
                              config['batch_size'], {
                                  'wN': config['wN'], 'w0': config['w0'], 'w1': config['w1'],
                                  'w2': config['w2'], 'wW': config['wW']
                              }, **config['dataloader_kwargs']).cache().prefetch(tf.data.AUTOTUNE)

    ds_val = get_dataloader(config['dataloader'], DATA_ROOT, config['val_years'], "train",
                            config['batch_size'], {'wN':1, 'w0':1, 'w1':1, 'w2':1, 'wW':1}, **config['dataloader_kwargs'])
    ds_val = ds_val.cache().prefetch(tf.data.AUTOTUNE)

    x, _, _ = next(iter(ds_train))
    in_shape = (120, 240, get_shape(x)[-1])
    c_shape = (120, 240, x['coordinates'].shape[-1])
    model = build_model(model=config['model'], shape=in_shape, c_shape=c_shape,
                        input_variables=config['input_variables'],
                        start_filters=config['start_filters'], l2_reg=config['l2_reg'],
                        dropout_rate=config['dropout_rate'])

    loss = lambda y_true, y_pred: (
        0.7 * tversky_loss(0.5, 0.5)(y_true, y_pred) + 0.3 * focal_loss(2.0, 0.85)(y_true, y_pred))

    lr_schedule = CosineDecayRestarts(config["learning_rate"], config["first_decay_steps"], t_mul=2.0, m_mul=0.9)

    opt = AdamW(learning_rate=lr_schedule, weight_decay=config["weight_decay"], beta_1=0.9, beta_2=0.999)

    metrics = [keras.metrics.AUC(from_logits=False, curve='PR', name='AUCPR', num_thresholds=500),
               tfm.F1Score(from_logits=False, name='F1'), tfm.BinaryAccuracy(from_logits=False),
               tfm.Precision(from_logits=False), tfm.Recall(from_logits=False), FalseAlarmRate(name='FalseAlarmRate'),
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(os.path.join(expdir, 'model_{epoch:03d}.keras'), monitor='val_AUCPR'),
    ...
    model.fit(ds_train, validation_data=ds_val, epochs=config['epochs'], callbacks=callbacks, verbose=1)
