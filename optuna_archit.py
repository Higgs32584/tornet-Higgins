...
def build_model(
    ...
    **config,
    ...
    # ResNet-style path
    path_a, c_out = wide_resnet_block(
        x,
        cin,
    ...
    attention_map = Dropout(rate=dropout_rate, name="spatial_attention_dropout")(
        attention_map
    )
    ...
    # Dense head
    x = Dense(128, activation="relu", kernel_regularizer=keras.regularizers.l2(l2_reg))(
        x
    )
    x = Dropout(dropout_rate)(x)
    x = Dense(64, activation="relu", kernel_regularizer=keras.regularizers.l2(l2_reg))(
        x
    )
...
def objective(trial):
    ...
    config.update(
        {
            ...
            "learning_rate": round(
                trial.suggest_float("learning_rate", 1e-6, 1e-5, log=True), 5
            ),
            ...
            "label_smoothing": round(
                trial.suggest_float("label_smoothing", 0.05, 0.20), 5
            ),
        }
    )
    ...
    try:
        model.compile(
            ...
            metrics=[
                keras.metrics.AUC(
                    from_logits=False, curve="PR", name="AUCPR", num_thresholds=1000
                ),
