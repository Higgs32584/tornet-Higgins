...
def read_file(
    ...
    with xr.open_dataset(f, chunks="auto") as ds:
        ...
        data["category"] = np.array(
            [{"TOR": 0, "NUL": 1, "WRN": 2}.get(category, 1)], dtype=np.int64
        )
  ...
  """
            ...
            features=tfds.features.FeaturesDict(
                {
                    ...
                    "DBZ": tfds.features.Tensor(
                        shape=(4, 120, 240, 2), dtype=np.float16, encoding="zlib"
                    ),
    ...
    def _generate_examples(self, path):
        ...
        try:
            ...
            raise ValueError(
                f"Expected a year (integer) as directory name, but got: {path.name}"
            )
