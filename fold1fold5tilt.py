...
def build_model(
    ...
    n_sweeps = shape[2]  # Number of tilts

    # Stack normalized radar variables: (H, W, T, V)
    x_vars = [normalize(inputs[v], v) for v in input_variables]  # each: (H, W, T)
    x = keras.layers.Lambda(lambda tensors: tf.stack(tensors, axis=-1))(
        x_vars
    )  # (H, W, T, V)

    # Replace NaNs with fill value
    x = FillNaNs(background_flag)(x)  # (H, W, T, V)

    # Add range-folded mask: (H, W, T, 1)
    if include_range_folded:
        range_folded = keras.Input(
            shape[:2] + (n_sweeps,), name="range_folded_mask"
        )  # (H, W, T)
        inputs["range_folded_mask"] = range_folded
        range_folded_exp = keras.layers.Lambda(lambda x: tf.expand_dims(x, axis=-1))(
            range_folded
        )  # (H, W, T, 1)
        x = keras.layers.Concatenate(axis=-1)([x, range_folded_exp])  # (H, W, T, V+1)

    # Add coordinates across tilts
    coords = keras.Input(c_shape, name="coordinates")  # (H, W, C)
    inputs["coordinates"] = coords
    coords_exp = keras.layers.Lambda(
        lambda x: tf.tile(tf.expand_dims(x, axis=3), [1, 1, 1, shape[2], 1]),
        output_shape=(shape[0], shape[1], shape[2], c_shape[2]),
    )(coords)
    x = keras.layers.Concatenate(axis=-1)([x, coords_exp])  # (H, W, T, V+1+C)

    # Optional: use Conv3D here for tilt modeling
    # x = Conv3D(...)(x)
    ...
    x = multi_dilated_attention_block(x, filters=mid_filters, kernel_regularizer=l2_reg)
    attn = multi_dilated_attention_head(x)
    x = Multiply()([x, attn])

    # Pooling and MLP head
    x_avg = GlobalAveragePooling3D()(x)
    x_max = GlobalMaxPooling3D()(x)
...
def se_block(x, ratio=16, kernel_initializer="glorot_uniform", name=None):
    filters = x.shape[-1]
    se = GlobalAveragePooling3D(name=f"{name}_gap" if name else None)(x)
    se = Dense(
        filters // ratio,
        activation="relu",
...
def pseudo_conv3d(x, filters, kernel_size=3, strides=1, **kwargs):
    # Conv2D over spatial dimensions
    x = TimeDistributed(Conv2D(filters, kernel_size, strides=strides, padding="same"))(
        x
    )
    x = Conv3D(filters, kernel_size=(1, 1, 3), padding="same")(x)
    return x
...
def multi_dilated_attention_head(x, dilation_rates=[1, 2, 4], name_prefix="attn"):
    ...
    avg_pool = GlobalAveragePooling3D(name=f"{name_prefix}_avg_pool")(x)
    max_pool = GlobalMaxPooling3D(name=f"{name_prefix}_max_pool")(x)
...
def multi_dilated_attention_block(
    ...
    # Context from input
    avg_pool = GlobalAveragePooling3D(name=f"{name_prefix}_gap")(x)
    max_pool = GlobalMaxPooling3D(name=f"{name_prefix}_gmp")(x)
