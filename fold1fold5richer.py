...
class WarmUpCosine(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step):
        step = step + self.start_step  # Offset
...
def main(config):
    ...
    nn.load_weights(
        "tornado_baseline_fold_1_250603194706-None-None/epoch_010_valAUCPR_0.6305.keras"
    )
    from_logits = False
    ...
    base_optimizer = keras.optimizers.Adam(learning_rate=lr, clipnorm=1.0)
    opt = LossScaleOptimizer(base_optimizer, dynamic=True)
    metrics = [
        keras.metrics.AUC(
