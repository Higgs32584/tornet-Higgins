...
class WarmUpExpDecay(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __call__(self, step):
        ...
        decay_fn = tf.keras.optimizers.schedules.ExponentialDecay(
            self.initial_lr, self.decay_steps, self.decay_rate, staircase=False
        )
...
def build_model(
    ...
    easy_path = Conv2D(
        filters=hard_path.shape[-1],
        kernel_size=1,
        padding="same",
        name="project_easy_path",
    )(easy_path)

    one_minus_gate = Lambda(lambda x: 1.0 - x)(gate_score)
    x = Add()(
        [Multiply()([gate_score, hard_path]), Multiply()([one_minus_gate, easy_path])]
    )
...
def wide_resnet_block(
    x, c, filters=64, widen_factor=2, l2_reg=1e-6, drop_rate=0.1, stride=1
):
