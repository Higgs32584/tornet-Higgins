...
def main(config, model_path):
    ...
    ds_val = get_dataloader(
        dataloader, DATA_ROOT, val_years, "train", batch_size, config["weights"], **dataloader_kwargs
    )
    ...
    # Load model
    nn = tf.keras.models.load_model(model_path, compile=False, safe_mode=False)
    ...
    for x_batch, y_batch, _ in tqdm(ds_val, desc="Predicting"):
        ...
        y_pred_prob.append(nn.predict(x_batch, verbose=0))  # Set verbose=0 to suppress per-batch logs
    ...
    print(f"Best F1-Score Threshold: {thresholds[best_f1_idx]:.4f} (F1: {f1[best_f1_idx]:.4f})")
    print(f"Best Threat Score Threshold: {thresholds[best_ts_idx]:.4f} (TS: {ts[best_ts_idx]:.4f})")
