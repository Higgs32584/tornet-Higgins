...
def check_layer_utilization(model_path):
    ...
    if isinstance(input_shapes, dict):  # Handle named inputs
        ...
        random_inputs = [
            np.random.rand(*[dim if dim is not None else 1 for dim in shape]).astype(
                np.float32
            )
        ...
        raise ValueError(
            f"Unsupported input structure: {input_shapes}. Expected dictionary-based named inputs or a tuple of input shapes."
        )
    ...
    activations = activation_model(
        random_inputs, training=False
    )  # Use function call instead of .predict()

    # Check utilization per layer
    for layer, activation in zip(model.layers, activations):
        if isinstance(activation, np.ndarray):
            active_neurons = np.sum(
                np.abs(activation) > 1e-5
            )  # Count nonzero activations
