...
def build_model(model='wide_resnet',shape:Tuple[int]=(120,240,2),
    ...
    if model == 'wide_resnet':
        x, c = wide_resnet_block(x, c, filters=start_filters, widen_factor=2, l2_reg=l2_reg,nconvs=ncovs, drop_rate=dropout_rate)
        x, c = wide_resnet_block(x, c, filters=start_filters*2, widen_factor=2, l2_reg=l2_reg,nconvs=ncovs, drop_rate=dropout_rate)
        x, c = wide_resnet_block(x, c, filters=start_filters*4, widen_factor=2, l2_reg=l2_reg,nconvs=ncovs, drop_rate=dropout_rate)
        x=se_block(x)
    x = Conv2D(128, 3, padding='same', activation='relu')(x)
    x = BatchNormalization()(x)
    attention_map = Conv2D(1, 1, activation='sigmoid', name='attention_map')(x)  # shape (B, H, W, 1)
    attention_map = Dropout(rate=dropout_attn, name='attention_dropout')(attention_map)
...
DEFAULT_CONFIG={"epochs":20, 
                "input_variables": ["DBZ", "VEL", "KDP", "ZDR","RHOHV","WIDTH"], 
                "train_years": [2014,2015,2016], 
                "val_years": [2017], "batch_size": 128
                , "model": "wide_resnet", 
                "start_filters": 48, 
                ...
                "l2_reg": 1.59e-7,
                "alpha": 0.744741158,
                "alpha_tversky": 0.441848158,
                "beta_tversky": 0.487643395,
                "focal_gamma": 1.839634212,
                "focal_alpha": 0.828034992,
                "weights":{
                "wN": 0.05069,
                    "w0": 0.55924,
                    "w1": 3.49593,
...
def objective(trial):
    ...
    logging.info(f"Starting trial with base config: {config}")

    # 🔧 Tune key hyperparameters
    config.update({
        "t_mul":trial.suggest_float("t_mul", 1.4, 2.6),
        "m_mul":trial.suggest_float("m_mul", 0.6, 1.2),
        "beta_1":trial.suggest_float("beta_1", 0.6, 1.2),
        "beta_2":trial.suggest_float("beta_2", 0.6, 1.2),
        "epsilon":trial.suggest_float("epsilon", 7e-8, 1.3e-7),
        "dropout_attn":trial.suggest_float("dropout_attn", 0.0, 0.2),
    })
    ...
    loss = combo_loss(alpha=config['alpha'],
                      alpha_tversky=config['alpha_tversky'],
                      beta_tversky= config['beta_tversky'],
    ...
    lr_schedule = CosineDecayRestarts(
        ...
        t_mul=config['t_mul'],               # each cycle doubles
        m_mul=config['m_mul']              # restart peak decays slightly
    )
