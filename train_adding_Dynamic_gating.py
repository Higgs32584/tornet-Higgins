...
class GateScoreMonitor(tf.keras.callbacks.Callback):
    def __init__(self, model, gate_layer_name="gate_score", val_data=None):
        ...
        self.gate_output_fn = tf.keras.backend.function(
            model.inputs, [self.gate_layer.output]
        )
...
def build_model(
    ...
    # STEP 1: Early classifier head
    early_feat = GlobalAveragePooling2D()(
        normalized_inputs
    )  # Or intermediate feature map
    early_class_logits = Dense(16, activation="relu")(early_feat)
    early_class_prob = Dense(1, activation="sigmoid", name="early_class_prob")(
        early_class_logits
    )

    # STEP 2: Compute uncertainty (distance from 0.5)
    uncertainty = Lambda(lambda x: tf.abs(x - 0.5), name="uncertainty")(
        early_class_prob
    )  # Shape: (B, 1)

    # STEP 3: Original gate features
    ...
    # STEP 4: Concatenate uncertainty with gate features
    gate_input = keras.layers.Concatenate(name="concat_gate_uncertainty")(
        [gate_pool, uncertainty]
    )

    # STEP 5: Compute gate score (now class-aware)
    gate_score = Dense(1, activation="sigmoid", name="gate_score")(gate_input)
