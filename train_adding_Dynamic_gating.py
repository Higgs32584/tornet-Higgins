...
def build_model(model='wide_resnet',shape:Tuple[int]=(120,240,2),
    ...
    early_feat = GlobalAveragePooling2D()(normalized_inputs)  # Or intermediate feature map
    early_class_logits = Dense(16, activation='relu')(early_feat)
    early_class_prob = Dense(1, activation='sigmoid', name='early_class_prob')(early_class_logits)

    # STEP 2: Compute uncertainty (distance from 0.5)
    uncertainty = Lambda(lambda x: tf.abs(x - 0.5), name="uncertainty")(early_class_prob)  # Shape: (B, 1)
    ...
    gate_input = keras.layers.Concatenate(name="concat_gate_uncertainty")([gate_pool, uncertainty])
