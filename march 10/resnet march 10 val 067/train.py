...
def build_model_exp(
    ...
    if model == "vgg_gated":
        ...
        for multiplier in filter_multipliers:
            x, c = inception_block(
                x, c, filters=start_filters * multiplier, l2_reg=l2_reg
            )
        ...
        x, c = dense_block(
            x, c, filters=start_filters, ksize=3, l2_reg=l2_reg, drop_rate=0.1
        )
        x, c = dense_block(
            x, c, filters=start_filters * 2, ksize=3, l2_reg=l2_reg, drop_rate=0.1
        )
        ...
        x, c = resnet_block(
            x, c, filters=start_filters * 4, ksize=3, l2_reg=l2_reg, drop_rate=0.1
        )
    if head == "mlp":
        # MLP head
...
def resnet_block(x, c, filters=64, ksize=3, l2_reg=1e-6, drop_rate=0.0):
    ...
    if x.shape[-1] != shortcut_x.shape[-1]:  # Check if channels mismatch
        shortcut_x = Conv2D(x.shape[-1], (1, 1), padding="same")(
            shortcut_x
        )  # Match channels

    x = Add()([x, shortcut_x])  # Now the shapes are aligned
...
def fusion_block(x1, x2, method="concat"):
    ...
    """
    if method == "concat":
        return Concatenate()([x1, x2])  # Simple Concatenation
        ...
        fusion_input = Concatenate()([x1, x2])
        fusion_input = Reshape((1, 1, fusion_input.shape[-1]))(fusion_input)
        fusion_conv = Conv2D(128, (1, 1), activation="relu")(fusion_input)
        fusion_conv = Conv2D(64, (1, 1), activation="relu")(fusion_conv)
        return Flatten()(fusion_conv)

    return x1  # Default to identity (no fusion)


def vgg_block(
    x, c, filters=64, ksize=3, n_convs=2, l2_reg=1e-6, drop_rate=0.0, se_block=False
):
...
def se_block(x, filters, ratio=8):
    ...
    se = Conv2D(
        filters // ratio, (1, 1), activation="swish", kernel_initializer="he_normal"
    )(se)
    se = Conv2D(filters, (1, 1), activation="sigmoid", kernel_initializer="he_normal")(
        se
    )
...
def inception_block(x, c, filters=128, l2_reg=1e-6, drop_rate=0.1):
    ...
    target_size = tf.shape(branch2)[
        1:3
    ]  # Get the pooled shape (branch2 is 1/2 of original)
    branch1 = Resizing(target_size[0], target_size[1])(branch1)  # Downsample branch1
    branch3 = Resizing(target_size[0], target_size[1])(
        branch3
    )  # Ensure branch3 matches
...
def main(config):
    ...
    lr = keras.optimizers.schedules.CosineDecayRestarts(
        initial_learning_rate=1e-4, first_decay_steps=10, t_mul=2.0, m_mul=0.9
    )
    opt = keras.optimizers.AdamW(learning_rate=lr, weight_decay=1e-4, amsgrad=True)
    from_logits = False
