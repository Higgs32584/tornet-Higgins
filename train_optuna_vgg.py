...
def get_optimized_batch_size(model_name, start_bs=128, min_bs=8, max_attempts=10):
    ...
    while attempt < max_attempts:
        try:
            ...
            ds_train = get_dataloader(DEFAULT_CONFIG['dataloader'], DATA_ROOT, DEFAULT_CONFIG['train_years'], "train",
                                      batch_size, None, **DEFAULT_CONFIG['dataloader_kwargs'])
            ...
            attempt += 1
    logging.error(f"Could not find feasible batch size for {model_name}. Using {batch_size}.")
    return batch_size
...
def build_model(model, shape: Tuple[int] = (120, 240, 2), c_shape: Tuple[int] = (120, 240, 2), input_variables: List[str] = ALL_VARIABLES, start_filters=48, l2_reg=0.003,drop_rate=.1):
    ...
    model_structure = {
        ...
        'vgg_3': [(2*start_filters, 2), (4*start_filters, 3), (8*start_filters, 3)],
        'vgg_4': [(2*start_filters, 2), (4*start_filters, 3), (8*start_filters, 3), (16*start_filters, 3)]
    }

    for filters, n_convs in model_structure[model]:
        x, c = vgg_block(x, c, filters=filters, ksize=3, l2_reg=l2_reg, n_convs=n_convs, drop_rate=drop_rate)
...
def get_optimized_batch_size(model_name, start_bs=128, min_bs=8, max_attempts=5):
    ...
    while attempt < max_attempts:
        try:
            ...
            ds_train = get_dataloader(DEFAULT_CONFIG['dataloader'], DATA_ROOT, 
                                      DEFAULT_CONFIG['train_years'], "train",
                                      batch_size, DEFAULT_CONFIG['weights'], **DEFAULT_CONFIG['dataloader_kwargs'])
            ...
            logging.warning(f"🚨 OOM at batch size {batch_size} for {model_name}, reducing by half...")
            batch_size = max(batch_size // 2, min_bs)
            attempt += 1

    logging.error(f"⚠️ Could not find feasible batch size for {model_name}. Using {batch_size}.")
...
def objective(trial):
    ...
    config["t_mul"] = trial.suggest_float("t_mul", 1.0, 3.0, log=True)  # Growth factor of decay period
    ...
    model = build_model(
        config["model"], shape=in_shapes, c_shape=c_shapes, 
        input_variables=config['input_variables'], 
        l2_reg=config["l2_reg"], start_filters=config["start_filters"]
    )
