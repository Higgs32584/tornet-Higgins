...
def read_file_optimized(f: str, variables: List[str] = ALL_VARIABLES, n_frames: int = 4) -> Dict[str, np.ndarray]:
    ...
    with xr.open_dataset(f, chunks="auto") as ds:  # Lazy loading with Dask
        ...
        for v in variables:
            data[v] = ds[v][-n_frames:, :, :, :].load().astype(np.float32)  # Only load required slices
        
        # Metadata processing
        data['range_folded_mask'] = ds['range_folded_mask'][-n_frames:, :, :, :].astype(np.float32).load()
        data['label'] = ds['frame_labels'][-n_frames:].astype(np.uint8).load()
        data['category'] = np.array([{'TOR': 0, 'NUL': 1, 'WRN': 2}[ds.attrs['category']]], dtype=np.int64)
        ...
        data['time'] = (ds.time[-n_frames:].astype(np.int64) // 1e9).astype(np.int64).load()

        # Tornado event timestamps
        if ds.attrs['ef_number'] >= 0 and 'tornado_start_time' in ds.attrs:
            data['tornado_start_time'] = np.array([pd.to_datetime(ds.attrs['tornado_start_time']).timestamp()], dtype=np.int64)
            data['tornado_end_time'] = np.array([pd.to_datetime(ds.attrs['tornado_end_time']).timestamp()], dtype=np.int64)
        else:
...
class TorNetBuilder(tfds.core.GeneratorBasedBuilder):
    ...
    # ✅ Explicitly define dataset name
    BUILDER_CONFIGS = [tfds.core.BuilderConfig(name=DATASET_NAME, version=tfds.core.Version("1.1.0"))]
    ...
    def _info(self) -> tfds.core.DatasetInfo:
        ...
        return self.dataset_info_from_configs(
            features=tfds.features.FeaturesDict({
                'DBZ': tfds.features.Tensor(shape=(4, 120, 240, 2), dtype=tf.float32, encoding='zlib'),
                'VEL': tfds.features.Tensor(shape=(4, 120, 240, 2), dtype=tf.float32, encoding='zlib'),
                'KDP': tfds.features.Tensor(shape=(4, 120, 240, 2), dtype=tf.float32, encoding='zlib'),
                'RHOHV': tfds.features.Tensor(shape=(4, 120, 240, 2), dtype=tf.float32, encoding='zlib'),
                'ZDR': tfds.features.Tensor(shape=(4, 120, 240, 2), dtype=tf.float32, encoding='zlib'),
                'WIDTH': tfds.features.Tensor(shape=(4, 120, 240, 2), dtype=tf.float32, encoding='zlib'),
                'range_folded_mask': tfds.features.Tensor(shape=(4, 120, 240, 2), dtype=tf.float32, encoding='zlib'),
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        splits = {f'train-{year}': catalog[catalog['split'] == f'train-{year}'] for year in range(2013, 2023)}
        splits.update({f'test-{year}': catalog[catalog['split'] == f'test-{year}'] for year in range(2013, 2023)})

        return [
            tfds.core.SplitGenerator(
                name=split_name,
                gen_kwargs={'file_paths': split_df['file'].tolist()}
