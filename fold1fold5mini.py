...
def build_model(
    ...
    n_sweeps = shape[2]
    normalized_vel = normalize(inputs["VEL"], "VEL")
    az_shear = keras.layers.Lambda(
        lambda v: v[..., 1] - v[..., 0], name="az_shear_from_normalized_vel"
    )(normalized_vel)
    az_shear = keras.layers.Reshape((*shape[:2], 1))(az_shear)
    ...
    def compute_divergence(v):
        ...
        dx = tf.pad(dx, [[0, 0], [0, 1], [0, 0], [0, 0]], mode="CONSTANT")
        dy = tf.pad(dy, [[0, 0], [0, 0], [0, 1], [0, 0]], mode="CONSTANT")

        div = dx + dy
        return div

    divergence = keras.layers.Lambda(
        compute_divergence, output_shape=(shape[0], shape[1], 1), name="divergence"
    )(normalized_vel)
    divergence = keras.layers.Reshape((*shape[:2], 1))(divergence)

    # Normalize the rest of the input variables
    normalized_vars = [
        normalize(inputs[v], v) if v != "VEL" else normalized_vel
        for v in input_variables
    ]

    # Concatenate all normalized variables plus az_shear
    x = keras.layers.Concatenate(axis=-1)(normalized_vars + [az_shear, divergence])
