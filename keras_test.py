...
from tensorflow.keras.layers import Dropout, BatchNormalization, Dense, GlobalAveragePooling2D, Reshape, multiply, Concatenate, Normalization
...
from keras.layers import Input, Normalization, Concatenate, Dropout, Dense, GlobalAveragePooling2D, MaxPool2D
from tornet.models.keras.layers import CoordConv2D, FillNaNs
# Input & Normalization
inputs = {v: Input(shape=(120, 240, 2), name=v) for v in ALL_VARIABLES}
norm_layers = [Normalization(mean=np.array(2 * [(CHANNEL_MIN_MAX[v][0] + CHANNEL_MIN_MAX[v][1]) / 2]), 
                                 variance=np.array(2 * [((CHANNEL_MIN_MAX[v][1] - CHANNEL_MIN_MAX[v][0]) / 2) ** 2]), 
                                                              name=f'Normalized_{v}') for v in ALL_VARIABLES]

x = Concatenate(axis=-1, name='Concatenate1')([l(inputs[v]) for l, v in zip(norm_layers, ALL_VARIABLES)])
...
for f, d in zip(filters, dropouts):
        x = CoordConv2D(f, (3, 3), strides=(2,2),padding="same",kernel_regularizer=keras.regularizers.l2(1e-6), activation="relu")(x)
...
# Create Model
model = build_model(shape=in_shapes,c_shape=c_shapes,start_filters=start_filters,l2_reg=l2_reg,input_variables=input_variables,head=head)
